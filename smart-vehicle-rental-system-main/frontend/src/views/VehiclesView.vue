<template>
  <div style="max-width:1280px; margin:0 auto; padding:28px 32px;">
    ...
    <div class="d-flex align-center justify-space-between mb-6">
      ...
      <div class="d-flex gap-2 flex-wrap">
        <v-chip
          ...
          size="small" @click="activeFilter = f.value" class="cursor-pointer"
        >{{ f.label }}</v-chip>
      </div>
    </div>

    <!-- AI Recommender -->
    <SmartRecommender v-if="currentUser?.role === 'CUSTOMER' && !loading && vehicles.length" :vehicles="vehicles" />
    ...
    <!-- Vehicle grid -->
    <v-row v-else>
      <v-col v-for="(v, i) in filteredVehicles" :key="v.id" cols="12" sm="6" md="4" lg="3">
        <v-card :class="`hover-lift -delay-${(i % 4) + 1}`" rounded="xl" elevation="1" height="100%" class="d-flex flex-column">
          ...
          <v-card-actions class="pa-3 flex-column gap-1">
            <v-btn variant="outlined" color="primary" block rounded="lg" size="small" :to="`/vehicles/${v.id}`" prepend-icon="mdi-eye">View Details</v-btn>
            <template v-if="v.status === 'AVAILABLE' && currentUser?.role === 'CUSTOMER'">
              <v-btn color="warning" block rounded="lg" size="small" prepend-icon="mdi-lightning-bolt" @click="startInstant(v)" elevation="0">Instant</v-btn>
              <v-btn color="primary" variant="tonal" block rounded="lg" size="small" prepend-icon="mdi-file-sign" @click="openContract(v)">Contract</v-btn>
            </template>
...
import SmartRecommender from '../components/SmartRecommender.vue'

export default {
  name: 'VehiclesView',
  components: { SmartRecommender },
  data() {
    return {
  ...
  mounted() {
    setTimeout(() => { this.vehicles = getVehicles(); this.loading = false }, 500)
  },
  methods: {
